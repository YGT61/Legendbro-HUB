...
TagsAndShit.ImageColor3 = Color3.fromRGB(80, 0, 0)
...
SideBarOptions.ImageColor3 = Color3.fromRGB(80, 0, 0)
...
SideBarOptions_2.ImageColor3 = Color3.fromRGB(80, 0, 0)
...
fedorabutton.ImageColor3 = Color3.fromRGB(160, 10, 10)
...
bar.ImageColor3 = Color3.fromRGB(160, 10, 10)
...
bar2.ImageColor3 = Color3.fromRGB(160, 10, 10)
...
bar3.ImageColor3 = Color3.fromRGB(160, 10, 10)
...
bar4.ImageColor3 = Color3.fromRGB(160, 10, 10)
...
THEBUTTON.MouseButton1Down:Connect(function()
	-- Universal: characterdeki tüm aksesuarların SpecialMesh'ini kaldır
	local char = game.Players.LocalPlayer.Character
	if not char then return end
	local accessoryNames = {"swordhalo","BladeMasterAccessory","ShadowBladeMasterAccessory","Black Demon Trident","White Demon Trident","Divine Aura","MeshPartAccessory","NGRWings"..game.Players.LocalPlayer.Name}
	-- Önce mevcut aksesuarları temizle (isimden bağımsız)
...
-- Universal: karakterdeki TÜM aksesuarları topla (isimden bağımsız)
local MPASword = {}
...
-- Universal Wing Sistemi: Karakterdeki mevcut aksesuarları otomatik kullan
-- Sol kanat offsets (3 parça)
local leftOffsets = {
	CF.N(2,-2,-1)*CF.A(0,0,0),
	CF.N(4.25,-1,-1)*CF.A(0,0,M.R(15)),
	CF.N(6.5,.5,-1)*CF.A(0,0,M.R(30)),
}
-- Sağ kanat offsets (7 parça)
local rightOffsets = {
	CF.N(-2,-2,-1)*CF.A(0,0,0),
	CF.N(-4.25,-1,-1)*CF.A(0,0,M.R(-15)),
	CF.N(-6.5,.5,-1)*CF.A(0,0,M.R(-30)),
...
local wingPieces = {}
local accIdx = 1
local totalPieces = #leftOffsets + #rightOffsets

-- Sol kanat parçaları
for i, offset in ipairs(leftOffsets) do
	local wp = WingPiece:Clone()
	if MPASword[accIdx] then
		for _,v in pairs(wp:GetChildren()) do
			if v:IsA("BasePart") then v.Transparency = 1 end
		end
		local athp = Instance.new("Attachment", wp.PrimaryPart)
		local atho = Instance.new("Attachment", wp.PrimaryPart)
		local hat = MPASword[accIdx]
		pcall(function()
			hat.Handle:FindFirstChildOfClass("AlignPosition").Attachment1 = athp
			hat.Handle:FindFirstChildOfClass("AlignOrientation").Attachment1 = atho
		end)
	...
	wp.Parent = leftWing
	local w = Weld(wp.PrimaryPart, Torso, offset)
	table.insert(wingPieces, {piece=wp, weld=w})
end

-- Sağ kanat parçaları
for i, offset in ipairs(rightOffsets) do
	local wp = WingPiece:Clone()
	if MPASword[accIdx] then
		for _,v in pairs(wp:GetChildren()) do
			if v:IsA("BasePart") then v.Transparency = 1 end
...
-- Dummy variables for backward compat (kullanılıyorsa hata vermesin)
local LWP1 = wingPieces[1] and wingPieces[1].piece or WingPiece:Clone()
LWP1.Parent = leftWing

-- Animasyon uyumluluğu için weld alias'ları (wingPieces tablosundan)
local function getWeld(i) return wingPieces[i] and wingPieces[i].weld or {C0=CFrame.new()} end
local LWP1W = getWeld(1)
local LWP2W = getWeld(2)
local LWP3W = getWeld(3)
local RWP1W = getWeld(4)
local RWP2W = getWeld(5)
local RWP3W = getWeld(6)
local RWP4W = getWeld(7)
local RWP5W = getWeld(8)
local RWP6W = getWeld(9)
local RWP7W = getWeld(10)
